...
// Adjust these scale factors to control the 'height' and 'frequency' of the hills/mountains
let noiseScale = 0.05; // Determines the frequency of the noise
let heightScale =  600;  // Determines the height variation caused by the noise
let noiseValue;

// Create points in a grid with noise applied
for ( y = 0; y <= segments; ++y) {
  for ( x = 0; x <= segments; ++x) {
    noiseValue = p5Instance.noise(x * noiseScale, y * noiseScale) * heightScale;
    let px = posx + x * xStride; // No noise applied to x-coordinate
    let py = posy + y * yStride + noiseValue; // Apply noiseValue as an offset to the y-coordinate
    cloth.addPoint(px, py);
  }
}
...
// for (x = 0; x < segments; ++x) {
//   cloth.pin(x);
 // Pin the entire top edge
 for (let x = 0; x <= segments; ++x) {
  let topEdgePointIndex = x; // Index of the point at the top edge
  cloth.pin(topEdgePointIndex);
}
  ...
  createCharCloth(posx, posy, w, h, segments, pinOffset, text ) {
    ...
    console.log("i am stick!", this.stickCount)
